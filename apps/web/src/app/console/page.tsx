    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
        <Panel title="Lifecycle distribution" right={`${data.counts.assets} TOTAL`}>
          <div className="vs-stat-strip grid-cols-2 md:grid-cols-4 xl:grid-cols-7">
            {STATUS_ORDER.map((status) => (
              <div key={status} className="p-3">
                {/*
                  No `truncate` here. Seven columns is tight, and "needs manual review" was rendering as
                  "needs manual re…" — an asset state is not a label you can abbreviate, because the
                  difference between review, manual review and revision is the whole point of the
                  breakdown. The cell wraps to a second line instead; grid rows stretch, so the strip
                  stays level.
                */}
                <div className="vs-label leading-snug">{status.replace(/_/g, ' ')}</div>
        ...
        <div className="space-y-4">
          ...
          <Panel title="Platform" right="FR-14.1">
            <div className="vs-stat-strip grid-cols-2">
