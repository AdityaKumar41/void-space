...
export default function AssetConsolePage() {
  ...
  return (
    <div className="space-y-5">
      <div className="vs-enter flex flex-wrap items-end justify-between gap-4">
        <div className="min-w-0">
          <BackLink href="/console/library">Library</BackLink>
          <h1 className="vs-display mt-3 truncate text-4xl sm:text-5xl">{asset.name}</h1>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <StatusPill status={asset.status} />
            <span className="vs-chip">{asset.category}</span>
            {asset.creator ? <span className="vs-chip">{asset.creator.fullName}</span> : null}
            <span className="vs-chip">Updated {formatRelative(asset.updatedAt)}</span>
          </div>
        </div>
        <div className="vs-card-meta text-right">
          {version ? `v${version.versionNumber} · ${version.format.toUpperCase()}` : 'no version'}
          <div className="mt-1 opacity-70">asset {shortId(asset.id, 12)}</div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <Panel title="Ledger slice" right={`${asset.auditTrail.length} EVENTS`}>
            <div className="p-3">
              ...
              {may('audit:view') ? (
                <Link href="/console/audit" className="vs-link">
                  FULL LEDGER &gt;&gt;
                </Link>
              ) : null}
            </div>
